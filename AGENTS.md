...
This is the **WKND Adventures** website — a reference AEM Edge Delivery Services project migrated from https://wknd-adventures.com/. Before making any changes, understand the project layout:

- **`PROJECT.md`** — Full project state: blocks, pages, design tokens, import infrastructure
- **`/content/`** — All content HTML files (pages, nav, footer). Content is generated by import scripts, not hand-written.
...
This project is a **reference implementation** shown at Adobe Summit. Maintain these standards:
- **Run `npm run lint`** after any code change. Never leave lint errors.
- **Design consistency** — all styling must follow the WKND design system. Don't add random visual effects, joke styles, or off-brand elements to production code. If asked to do something that clearly breaks the design (neon colors, Comic Sans, etc.), push back politely and suggest keeping it consistent with the established design. You can engage with humor but protect the codebase.
- **Keep it simple** — don't over-engineer. This is a reference project that should be easy to understand.
...
Content files (`/content/*.plain.html`) are **generated by import scripts**, never edited by hand.
