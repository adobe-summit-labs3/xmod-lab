...
- **Header/footer content files:** Nav and footer are content files at `/workspace/content/nav.plain.html` and `/workspace/content/footer.plain.html`. The `getContentRoot()` utility in `scripts.js` resolves the fragment path dynamically based on the page URL, so nav/footer work whether content is served from `/` or `/content/`.
